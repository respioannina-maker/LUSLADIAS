...
function notifyAndReload(msg="✅ Καταχωρήθηκε", delayMs=1200){
  toast(msg, true, delayMs);
  setTimeout(()=> location.reload(), delayMs);
}
function lockBtn(id, locked=true){
...
$("cmb_submit").onclick = async ()=>{
  ...
  try{
    ...
    if (lusScore !== ""){
      ...
      notifyAndReload("✅ CRF + D1 LUS καταχωρήθηκαν");
    } else {
      notifyAndReload("✅ CRF καταχωρήθηκε");
    }
  } finally {
    lockBtn("cmb_submit", false);
  }
};

// ====== Submit: LUS Daily (D2–D7/Exit) ======
$("lus_submit").onclick = async ()=>{
  lockBtn("lus_submit", true);
  try{
    const data = {
    ...
    if (res.ok) notifyAndReload("✅ LUS καταχωρήθηκε");
    else toast("❌ " + (res.error||"σφάλμα"), false, 5000);
  } finally {
    lockBtn("lus_submit", false);
  }
};

// ====== Submit: Outcomes ======
$("out_submit").onclick = async ()=>{
  lockBtn("out_submit", true);
  try{
    const data = {
      action: "outcome",
    ...
    const res = await postForm(data);
    if (res.ok) notifyAndReload("✅ Εκβάσεις καταχωρήθηκαν");
    else toast("❌ " + (res.error||"σφάλμα"), false, 5000);
  } finally {
    lockBtn("out_submit", false);
  }
};
